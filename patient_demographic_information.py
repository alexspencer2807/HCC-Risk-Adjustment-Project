...
def extract_target_value(member_id, patient_category, df):
    ...
    if not age_row.empty:
        if 'Institutional' in patient_category:
            target_value = age_row['Institutional'].values[0]
        elif 'PBDual' in patient_category:
            if 'Aged' in patient_category:
                target_value = age_row['Community, PBDual, Aged'].values[0]
            else:
                target_value = age_row['Community, PBDual, Disabled'].values[0]
        elif 'FBDual' in patient_category:
            if 'Aged' in patient_category:
                target_value = age_row['Community, FBDual, Aged'].values[0]
            else:
                target_value = age_row['Community, FBDual, Disabled'].values[0]
        elif 'NonDual' in patient_category:
            if 'Aged' in patient_category:
                target_value = age_row['Community, NonDual, Aged'].values[0]
            else:
                target_value = age_row['Community, NonDual, Disabled'].values[0]
        
        # Display the extracted values used for calculation
...
def calculate_adjusted_risk_score(raw_risk_score, normalization_factor=1.069, ma_coding_pattern=5.9 / 100):
    """
...
def display_and_sum_values(file_path_1, file_path_2):
    ...
    df_combined['Adjusted Risk Score'] = df_combined['Target Value_Demographic'] + df_combined['Target Value_Disease']
    
    # Display the results
    print("Target Values and Adjusted Risk Scores:")
    print(df_combined[['MemberID', 'Target Value_Demographic', 'Target Value_Disease', 'Adjusted Risk Score']])
    
    # Save to an Excel file
    df_combined.to_excel("combined_target_values.xlsx", index=False)

# Call the process_code_2 function with appropriate file paths
table_1_path = 'C:/Users/Spencerdm/OneDrive/Documents/ADT Project/Rate Announcement 2020.xlsx'
...
process_code_1(table_1_path, table_2_path_code_1, output_path_code_1)
